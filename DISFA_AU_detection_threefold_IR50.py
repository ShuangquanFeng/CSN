...
# sample bash command: 
# python DISFA_AU_detection_threefold_IR50.py --epochs 3 --batch_size 64 --gpu 0 --save_interval 1

parser = argparse.ArgumentParser(description='Evaluate the IR50 model on the DISFA dataset for AU detection using threefold cross-validation.')
...
def main():
    ...
    for fold in range(1, 4):
        ...
        train_dataset = AU_dataset_single_frames(subject_dict = {'DISFAleft': train_subs, 'DISFAright': train_subs}, transform = data_transforms['train'], label_type = "binary occurrence")
        val_dataset = AU_dataset_single_frames(subject_dict = {'DISFAleft': val_subs, 'DISFAright': val_subs}, transform = data_transforms['val'], label_type = "binary occurrence")
