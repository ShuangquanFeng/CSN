...
# sample bash command: 
# python UNBCMcMaster_AU_intensity_estimation_fivefold_CSN-IR50.py --merge_locs stage4 --epochs 3 --batch_size 64 --gpu 0 --save_interval 1

parser = argparse.ArgumentParser(description='Evaluate the CSN-IR50 model on the UNBCMcMaster dataset for AU intensity estimation using fivefold cross-validation.')
...
def main():
    ...
    for fold in range(1, 6):
        ...
        model = iresnet_Siamese(n_layers=N_LAYERS, n_features = n_AUs * (MAX_INTENSITY + 1), merge_locs = merge_locs, pretrained = True, weights_path = FACE_RECOGNITION_MODEL_PATH)
        model = model.to(device)
        torch.cuda.device(args.gpu)
        
        # Initialize the dataset and dataloader
        print("Initializing Datasets and Dataloaders...")
        train_dataset = AU_dataset_frame_pairs(subject_dict = {'UNBCMcMaster': train_subs}, transform = data_transforms['train'], label_type = "intensity")
        val_dataset = AU_dataset_frame_pairs(subject_dict = {'UNBCMcMaster': val_subs}, transform = data_transforms['val'], label_type = "intensity")
