...
# sample bash command: 
# python DISFA_AU_intensity_estimation_threefold_CSN-IR50.py --merge_locs stage4 --epochs 3 --batch_size 64 --gpu 0 --save_interval 1

parser = argparse.ArgumentParser(description='Evaluate the CSN-IR50 model on the DISFA dataset for AU intensity estimation using threefold cross-validation.')
...
def main():
    ...
    for fold in range(1, 4):
        ...
        train_dataset = AU_dataset_frame_pairs(subject_dict = {'DISFAleft': train_subs, 'DISFAright': train_subs}, transform = data_transforms['train'], label_type = "intensity")
        val_dataset = AU_dataset_frame_pairs(subject_dict = {'DISFAleft': val_subs, 'DISFAright': val_subs}, transform = data_transforms['val'], label_type = "intensity")
