...
# sample bash command: 
# python UNBCMcMaster_AU_detection_fivefold_IR50.py --epochs 3 --batch_size 64 --gpu 0 --save_interval 1

parser = argparse.ArgumentParser(description='Evaluate the IR50 model on the UNBCMcMaster dataset for AU detection using fivefold cross-validation.')
...
def main():
    sel_AUs = UNBCMCMASTER_AUS_D.copy()
    n_AUs = len(sel_AUs)
    ...
    task = "UNBCMcMaster_AU_detection_fivefold"
    ...
    for fold in range(1, 6):
        ...
        model = iresnet_basic(n_layers=N_LAYERS, n_features = n_AUs, pretrained = True, weights_path = FACE_RECOGNITION_MODEL_PATH)
        model = model.to(device)
        torch.cuda.device(args.gpu)
        
        # Initialize the dataset and dataloader
        print("Initializing Datasets and Dataloaders...")
        train_dataset = AU_dataset_single_frames(subject_dict = {'UNBCMcMaster': train_subs}, transform = data_transforms['train'], label_type = "binary occurrence")
        val_dataset = AU_dataset_single_frames(subject_dict = {'UNBCMcMaster': val_subs}, transform = data_transforms['val'], label_type = "binary occurrence")
