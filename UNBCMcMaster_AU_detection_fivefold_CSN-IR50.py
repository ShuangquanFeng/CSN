...
# sample bash command: 
# python UNBCMcMaster_AU_detection_fivefold_CSN-IR50.py --merge_locs stage4 --epochs 3 --batch_size 64 --gpu 0 --save_interval 1

parser = argparse.ArgumentParser(description='Evaluate the CSN-IR50 model on the UNBCMcMaster dataset for AU detection using fivefold cross-validation.')
...
def main():
    ...
    for fold in range(1, 6):
        ...
        train_dataset = AU_dataset_frame_pairs(subject_dict = {'UNBCMcMaster': train_subs}, transform = data_transforms['train'], label_type = "binary occurrence")
        val_dataset = AU_dataset_frame_pairs(subject_dict = {'UNBCMcMaster': val_subs}, transform = data_transforms['val'], label_type = "binary occurrence")
