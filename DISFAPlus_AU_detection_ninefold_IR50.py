...
# sample bash command: 
# python DISFAPlus_AU_detection_ninefold_IR50.py --epochs 3 --batch_size 64 --gpu 0 --save_interval 1

parser = argparse.ArgumentParser(description='Evaluate the IR50 model on the DISFA+ dataset for AU detection using ninefold cross-validation.')
...
def main():
    ...
    task = "DISFAPlus_AU_detection_ninefold"
    ...
    for fold in range(1, 10):
        ...
        train_dataset = AU_dataset_single_frames(subject_dict = {'DISFAPlus': train_subs}, transform = data_transforms['train'], label_type = "binary occurrence")
        val_dataset = AU_dataset_single_frames(subject_dict = {'DISFAPlus': val_subs}, transform = data_transforms['val'], label_type = "binary occurrence")
